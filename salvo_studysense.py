...
feature_cols = []
target_col = None
formula_str = "No formula generated yet."
formula_coefs = {}
...
general_file = r"C:\Users\malle\Downloads\salvo\salvo\raw_data.xlsx"
...
def analyze_dataset_and_generate_formula(file_path):
    ...
    global formula_str, formula_coefs, formula_intercept, data_analytics_summary
    ...
    summary_lines = [
        ...
        f" Missing Values Detected: {sum(null_counts.values())}"
    ]
    ...
    formula_str = f"Final Marks = {formula_intercept:.4f} " + " ".join(terms)
...
try:
    prediction_data, current_mapping = analyze_dataset_and_generate_formula(general_file)
except Exception:
...
class studysenseapp:
    def __init__(self, root):
        ...
        title_label = tk.Label(
            root, text="STUDYSENSE", 
            font=("Arial", 15, "bold"), bg="#FFFFFF", fg="#111111"
        )
        title_label.pack(pady=(12, 2))

        self.dataset_status = tk.Label(
            root, text=f"Active Loaded File: {os.path.basename(prediction_data)} ({len(df_current) if df_current is not None else 0} rows)", 
    ...
    def upload_new_data(self):
        ...
        prediction_data= {os.path.basename(file_path)}
        if file_path:
            try:
    ...
    def use_default_data(self):
        try:
            name, mapping = analyze_dataset_and_generate_formula(general_file)
            self.dataset_status.config(text=f"Active Loaded File: {os.path.basename(general_file)} ({len(df_current)} rows)")
            self.analytics_label.config(text=f"{data_analytics_summary}\n\n Derived Formula:\n{formula_str}")
    ...
    def predict(self):
        ...
        if prev >= 70.0 and att <= 60.0:
            ...
            note_text = f" Score calculated dynamically using the formula derived from '{os.path.basename(prediction_data)}'."
        ...
        if matches:
            ...
            updated_df.to_excel(prediction_data, index=False)
            
            analyze_dataset_and_generate_formula(prediction_data)
            self.analytics_label.config(text=f"{data_analytics_summary}\n\n Derived Formula:\n{formula_str}")
            
            messagebox.showinfo(
                "Data Saved", 
                f"Thank you! Entry saved to '{os.path.basename(prediction_data)}' and formula updated."
            )
        else:
